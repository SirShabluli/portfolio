...
export default function DayNightToggle({
  ...
  return (
    <div style={{ transform: `scale(${scale})` }} className="origin-center">
      <button
        onClick={onToggle}
        className={`relative w-20 h-10 rounded-full transition-colors duration-500 origin-center cursor-pointer group ${
          pulse ? "pulse-toggle" : ""
        } ${
          isDark
            ? "bg-black border border-[#CCDEE2] shadow-[0_0_0_1px_#2B6CB5,inset_0_0_0_1px_#2B6CB5]"
            : "bg-[#23577A] border border-[#E4EBFF] shadow-[0_0_0_1.1px_#E4EBFF,inset_0_0_0_1px_transparent]"
        }`}
      >
        ...
        <span
          className={`absolute top-1/2 -translate-y-1/2 w-7 h-7 rounded-full transition-all duration-200 flex items-center justify-center ${
            isDark
              ? "left-11 group-hover:left-[2.6rem] bg-black border border-[#FEDCBB] shadow-[0_0_0_1px_#EC2426,inset_0_0_0_1px_#EC2426]"
              : "left-1 group-hover:left-[0.3rem] bg-white border border-transparent shadow-none"
          }`}
        >
          <span
